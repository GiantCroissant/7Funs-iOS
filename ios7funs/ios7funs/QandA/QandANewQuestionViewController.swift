...
class QandANewQuestionViewController: UIViewController {
  ...
  @IBAction func onAddQuestionClick(sender: UIButton) {
    if let token = LoginManager.token {
      ...
      let systemName = VersionUtils.getSystemName()
      let systemVersion = VersionUtils.getSystemVersion()
      let shortVersion = VersionUtils.getShortBuildVersion()
      let buildVersion = VersionUtils.getBuildVersion()
      let remark = "\(systemName) \(systemVersion) ; \(shortVersion) \(buildVersion)"

      self.showToastIndicator()
      QandAManager.sharedInstance.postQuestion(
        token,
        title: subject,
        description: content,
        remark: remark,
