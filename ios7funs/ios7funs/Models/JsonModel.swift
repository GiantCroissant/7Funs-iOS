...
public struct TagJsonObject {
  ...
  public func toDBModel() -> RecipeTag {
    var recipeIds = [Int]()
    self.taggings?.forEach { recipeIds.append($0.taggableId) }

    let recipeTag = RecipeTag()
    recipeTag.id = self.id
    recipeTag.name = self.name
    recipeTag.recipeIds = recipeIds
    return recipeTag
  }
}
